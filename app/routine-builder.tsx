  ...
  return (
    <Screen>
      <ScrollView contentContainerStyle={{ padding: 24, paddingBottom: 40 }}>
        <Text className="text-sm text-muted">{t("building")}</Text>
        <Text className="text-2xl font-bold text-ink">{t(rec.mudra.name)}</Text>
        {rec.reason ? (
          <Text className="mt-2 text-sm text-muted">{rec.reason}</Text>
        ) : null}

        <View className="mt-8">
          <SectionTitle>{t("what_practice")}</SectionTitle>
          <TimePicker value={time} onChange={setTime} />
        </View>

        <View className="mt-8">
          <SectionTitle>{t("session_length")}</SectionTitle>
